...
@section('content')
    <div class="container-fluid">
        ...
        @elseif(session('success_post_delete'))
            <div class="alert alert-success">
                {{ session('success_post_delete') }}
            </div>
        @elseif(session('success_comment_delete'))
            <div class="alert alert-success">
                {{ session('success_comment_delete') }}
            </div>
        @elseif(session('success_news_delete'))
            <div class="alert alert-success">
                {{ session('success_news_delete') }}
            </div>
        @endif
        <div class="row">
                                    ...
                                    <tr class="post-row">
                                      ...
                                      <td>  {!! nl2br(htmlspecialchars(mb_substr($post->message, 0, 10) . (mb_strlen($post->message) > 10 ? '...' : ''))) !!}</td>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('posts.delete', ['postId' => $post->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @foreach($news as $new)
                                    <tr class="post-row">
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('news.delete', ['newsId' => $new->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
